...
<!-- ecosystem:start -->
OpenClaw is at **390,547★** and Hermes Agent at **249,062★**, pulled from the GitHub API on 2026-09-26. Grok
Bot is a closed product with no comparable figure: the largest repo in its third-party ecosystem is xAI's own
marketplace at **267★**, 46 days after launch. Numbers this size move daily, so treat them as a snapshot.
<!-- ecosystem:end -->
...
Read from each repository's own file tree on 2026-09-26. Marketplace entries are read at the commit xAI pins, not at HEAD.
...
- [cosvoice-mcp](https://github.com/andyfathollahi/cosvoice-mcp) by [CosVoice](https://cosvoice.com) — a real phone number and email for your Bot over a hosted MCP server (OAuth by URL, no key): it places calls for you, books restaurants by phone, answers missed calls, and returns a written summary of every call. **[production]**
...
<!-- marketplace:start -->
xAI runs an [official plugin marketplace](https://github.com/xai-org/plugin-marketplace) — 29 vendor plugins in
`.grok-plugin` format, the same plugin surface Grok Bot inherits under Cursor's plugin/MCP policy. All
29 are listed below, each resolved at the exact commit the marketplace pins.
<!-- marketplace:end -->
